...
from mycrm.permission_list import perm_dic
        ...
        if per_url_name == current_url_name: #matches current request url
            if per_method == request.method: #matches request method
                ...
                for k,v in perm_kwargs.items():
                    ...
                    if arg_val == str(v): #匹配上特定的参数 及对应的 参数值， 比如，需要request 对象里必须有一个叫 user_id=3的参数
                        kwargs_matched = True
                    else:
                        kwargs_matched = False
                        break # 一个参数匹配不成功，则判定为假，退出该循环。
                else:
                    kwargs_matched = True
...
def check_permission(func):
    ...
    def inner(*args,**kwargs):
        if not perm_check(*args,**kwargs):
            request = args[0]
            return render(request,'kd/page_403.html')
