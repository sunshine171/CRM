...
def all(current_order_field):
    # print("pppppppp",list(current_order_field.values()))
    return list(current_order_field.values())[0] if current_order_field else ''


@register.simple_tag#gai
...
@register.simple_tag
def display_all_related_objs(obj):#这里时递归关系对象删除，跟递归评论应该很像，不是太明白
    """显示要被删除的所有关联的对象"""
